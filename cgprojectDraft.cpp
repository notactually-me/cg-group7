...
float cpos1=200,cpos2=400,cpos3=-250,cpos4=50,cpos5=490,sp1=0.6f,bul=0, tpos = 500, jpos = 500, tankpos = 0, canpos =690, _siren = 2, helipos = 300;;
float sun_x=450 ,sun_y=480,sp_x=0.5f,sp_y=0.7f,plane_x=0.0f, plane_speed=4.0f,_rain = 0.0, truckSpeed = 1.0f, jeepSpeed = 2.0f, tankSpeed = 1.0f, canSpeed = 20.0f, sirenspeed = 2.0f, helispeed = 3.0f;

bool rainday = false, day = true,fire=true, night = false, thunder = false;
...
void canonBall()
{
    ...
    glScalef(0.4,0.4,0);
    glRotatef(-25.0,1,1,1);
    glColor3ub(0,0,0);
    circle(canpos,225,20);
    glPopMatrix();
}
...
void canon()
{
    ...
    if (fire)canonBall();
    glTranslatef(0,320,0);
    if(day) glColor3ub(90,112,88);
    if(!day) glColor3ub(43,77,66);
    glScalef(0.4,0.4,0);
    glRotatef(-25.0,1,1,1);
...
void Helicopter()
{
    ...
    glScalef(0.6,0.6,0);
    glTranslatef(helipos, 0,0);
    glBegin(GL_QUADS);      //Helicopter

    if(day)glColor3ub(63,87,44);        //base quad
    if(!day)glColor3ub(32,61,61);        //base quad
    ...
    if(!day) glColor3ub(190,183,41);     //Front
    if(day) glColor3ub(253,209,40);     //Front
    ...
    if(!day)glColor3ub(190,183,41);  //body
    if(day) glColor3ub(253,209,40);
    ...
    if(day)glColor3ub(63,87,44);        //base quad
    if(!day)glColor3ub(32,61,61);      //Rear Blade
...
void siren()
{
    ...
    glPushMatrix();
    glTranslatef(460,407,0);
    glRotatef(_siren, 0,1,0);
    glBegin(GL_TRIANGLES);

    glColor3ub(255,232,42);
    glVertex2f(0,0);
    glColor3ub(255,250,160);
    glVertex2f(100,20);
    glColor3ub(255,250,160);
    glVertex2f(100,-40);
...
void update(int value){
    ...
    ///canonBall
    if(canpos>2200) canpos = 690;
    canpos += canSpeed;

   //if(_siren>360) _siren = 0.0f;
    _siren += sirenspeed;

    ///helicopter control
    if (helipos<-200) helipos = 300;
    helipos -= helispeed;


}

//void Thunder1(int value)
//{
//    if(thunder)
//    {
//
